<?php
/**
 * 11:11 Decor — Photo Gallery Management Dashboard
 */
session_start();
...
<head>
    ...
    <title>Photo Gallery Management — 11:11 Decor</title>
...
$live_site_url = $is_local ? 'http://localhost:3000/gallery' : '/gallery';
?>
        <header>
            <div>
            ...
            <div class="header-actions">
                <a href="new-photo.php" class="btn-primary">+ Add New Photo</a>
        ...
        <div class="nav-tabs">
            ...
            <a href="gallery.php" class="nav-tab active">Photo Gallery</a>
        </div>

        <!-- ── Page Visibility Controls Card at the Top ── -->
        <?php require_once __DIR__ . '/visibility-card.php'; ?>

        <h3 style="margin-bottom: 1.25rem; color: #c9a96e; font-size: 1.15rem;">Photo Gallery Showcase</h3>
                ...
                <?php if (empty($items)): ?>
                    <tr>
                        <td colspan="6" style="text-align: center; padding: 3.5rem; color: #8a8275;">
                            No gallery photos found. Click "+ Add New Photo" above to upload your first image!
                        </td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr>
                            <td>
                                <?php if (!empty($item['src'])): ?>
                                    <img src="<?= htmlspecialchars($item['src']) ?>" alt="" class="thumb">
                                <?php else: ?>
                                    <div class="thumb" style="background:#333;"></div>
                                <?php endif; ?>
                            </td>
                            <td>
                                <strong style="color: #ffffff;"><?= htmlspecialchars($item['title']) ?></strong>
                            </td>
                            <td>
                                <span style="color: #c9a96e;"><?= htmlspecialchars($item['category'] ?? 'Weddings') ?></span>
                            </td>
                            <td style="color: #8a8275;">
                                <?= htmlspecialchars($item['aspectRatio'] ?? 'landscape') ?>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="actions">
                                    <a href="edit-photo.php?id=<?= $item['id'] ?>" class="action-link">Edit</a>
                                    <a href="delete-photo.php?id=<?= $item['id'] ?>" class="action-link action-delete" onclick="return confirm('Are you sure you want to delete this photo?');">Delete</a>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
