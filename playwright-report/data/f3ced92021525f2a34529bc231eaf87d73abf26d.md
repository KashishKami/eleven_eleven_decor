...
- Name: hero.spec.ts >> Hero Section (W-201) >> renders full-viewport hero section with headline and CTA buttons
- Location: tests\e2e\hero.spec.ts:4:7
...
Locator: locator('#hero').getByRole('link', { name: /view our work/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#hero').getByRole('link', { name: /view our work/i })
...
```ts
  ...
  2  | 
  3  | test.describe('Hero Section (W-201)', () => {
  4  |   test('renders full-viewport hero section with headline and CTA buttons', async ({ page }) => {
  5  |     await page.goto('/')
  6  | 
  7  |     const heroSection = page.locator('#hero')
  8  |     await expect(heroSection).toBeVisible()
  9  | 
  10 |     const heading = heroSection.locator('h1')
  11 |     await expect(heading).toBeVisible()
  12 |     const headingText = await heading.textContent()
  13 |     expect(headingText).toContain('Your Wish Our Creation')
  14 | 
  15 |     const planBtn = heroSection.getByRole('link', { name: /plan your event/i })
  16 |     await expect(planBtn).toBeVisible()
  17 | 
  18 |     const workBtn = heroSection.getByRole('link', { name: /view our work/i })
> 19 |     await expect(workBtn).toBeVisible()
     |                           ^ Error: expect(locator).toBeVisible() failed
  20 |   })
  21 | })
  22 | 
